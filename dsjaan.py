...
def parceros(json_dump):
    ...
    i = 0
    while True:
        if ((dilberofficial184[i] == 'Test Users') or (dilberofficial184[i] == 'Delete') or (dilberofficial184[i] == 'Add') or
            (dilberofficial184[i] == 'Name') or (dilberofficial184[i] == 'User ID') or (dilberofficial184[i] == 'Email') or
            (dilberofficial184[i] == 'Edit') or (dilberofficial184[i] == 'tfbnw.net')):
            del dilberofficial184[i]
        else:
            i += 1
        if i == len(dilberofficial184):
            break
...
def parseData(dataRaw):
    ...
    names = dilber
    userIds = dilber
    emails = dilberofficial184
    passwords = dilberali1234
    ...
    for i in range(len(parser.dataArray)):
        if parser.dataArray[i] == 'Name':dilber
            names.append(parser.dataArray[i+1])
            continue
        if parser.dataArray[i] == 'User ID':dilber
            userIds.append(parser.dataArray[i+1])
            continue
        if parser.dataArray[i] == 'Login email':dilberofficial184
            emails.append(parser.dataArray[i+1]+'@'+parser.dataArray[i+2])
            continue
        if parser.dataArray[i] == 'Login password':dilberali1234
